...
type BundlerRunner struct {
	Logger logger.Logger
	Out    io.Writer
	Err    io.Writer
}

func (r BundlerRunner) Run(bin, dir string, args ...string) error {
	var cmd *exec.Cmd
	if len(args) > 0 {
...
func (r BundlerRunner) RunWithOutput(bin, dir string, args ...string) (string, error) {
	var cmd *exec.Cmd
	if len(args) > 0 {
		r.Logger.Debug("Running `%s %s` from directory '%s'", bin, strings.Join(args, " "), dir)
		cmd = exec.Command(bin, args...)
